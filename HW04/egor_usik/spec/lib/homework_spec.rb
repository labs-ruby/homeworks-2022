  ...
  describe '#add_answer' do
    context 'when adds an answer' do
      it 'adds an answer' do
        expect { valid_homework.add_answer('Answer') }.to change { create.answers.count }.by(1)
      end
    end
  end
  ...
  describe '#mark_as' do
    context 'when marks as executing, done' do
      example 'returns value of the status EXECUTING (1)' do
        expect(valid_homework.mark_as!(HomeworkStatuses::EXECUTING)).to eq(1)
      end

      example 'returns value of the status DONE (2)' do
        expect(valid_homework.mark_as!(HomeworkStatuses::DONE)).to eq(2)
      end
    end
    ...
    context 'when marks as done then as rejected' do
      it 'changes the status to DONE and then to REJECTED' do
        create.mark_as!(HomeworkStatuses::DONE).then do
          expect(valid_homework.mark_as!(HomeworkStatuses::REJECTED)).to eq(3)
        end
      end
    end
    ...
    context 'when marks as rejected the homework is executing' do
      it 'returns an error' do
        valid_homework.mark_as!(HomeworkStatuses::EXECUTING).then do
          expect { valid_homework.mark_as!(HomeworkStatuses::REJECTED) }.to raise_error(StandardError)
        end
      end
    end
    ...
    context 'when marks as rejected after the homework was accepted' do
      it 'returns an error' do
        valid_homework.mark_as!(HomeworkStatuses::ACCEPTED).then do
          expect { valid_homework.mark_as!(HomeworkStatuses::REJECTED) }.to raise_error(StandardError)
        end
      end
    end
    ...
    context 'when marks as new the homework has a different status' do
      it 'returns an error' do
        valid_homework.mark_as!(HomeworkStatuses::EXECUTING).then do
          expect { valid_homework.mark_as!(HomeworkStatuses::NEW) }.to raise_error(StandardError)
        end
      end
    end
  end
